...
class TwitterProtocol(protocol.Protocol):
  current_state = None
  user_id = ''
  debug = False
  ...
  def connectionMade(self):
    ...
    self.current_state = self.factory.twitter_states.LOGIN
  ...
  def dataReceived(self, data):
    ...
    if self.current_state == self.factory.twitter_states.LOGIN:
      self.handle_LOGIN(data)
    elif self.current_state == self.factory.twitter_states.USER:
  ...
  def handle_LOGIN(self, data):
    ...
    if user in self.factory.user_logins and user_pass == self.factory.user_logins[user]:
      ...
      self.current_state = self.factory.twitter_states.USER
  ...
  def newMessage(self, message_type, sender='twitter'):
    new_msg = {}
...
class TwitterFactory(protocol.ServerFactory):
    protocol = TwitterProtocol
    
    #enumerations as state variables
    twitter_states = enum('LOGIN', 'USER')

    #list of "registered" users
    #can't use twitter server unless these logged in with one of these credentials
...
endpoints.serverFromString(reactor, 'tcp:8000').listen(TwitterFactory())
